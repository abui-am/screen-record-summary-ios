...
class CLIPTokenizer {
    ...
    init() throws {
        guard let mergesURL = Self.resourceURL(name: "clip-merges", extension: "txt") else {
            throw ClassifierError.modelLoadFailed("Missing clip-merges.txt in app bundle.")
        }
        guard let vocabURL = Self.resourceURL(name: "clip-vocab", extension: "json") else {
            throw ClassifierError.modelLoadFailed("Missing clip-vocab.json in app bundle.")
        }

        let bpeMergesTxt = try String(contentsOf: mergesURL)
        ...
        self.encoder = try {
            ...
            return try decoder.decode([String: Int].self, from: json)
        }()
